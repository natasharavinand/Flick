(async ($d3, $genres, $months, $headers) => {
  ...
  const monthDiv = mainContainer.append('div').attr('id', 'months');
  ...
  const genreSelect = genreDiv.append('select').attr('id', 'genreSelect').on('change', function(event) {
    const currentGenre = document.getElementById('genreSelect').value;
    const currentYear = document.getElementById('yearSelect').value;
    getMovieDataForYear(currentYear, currentGenre);
  });

  genreSelect.selectAll('option').data($genres).enter().append('option').text(function(d) {
    return d.name;
  ...
  const getMovieDataForYear = async (year, genre) => {
    ...
    monthDiv.selectAll('div').data($months).enter().append('a').attr('class', 'month-name').attr('href', function(d) {
      return `#${d}-${year}`
    }).text(function(d) {
      return d;
    });
    ...
    await asyncForEach($months, async (month, index) => {
      ...
      try {
        ...
        genre
          ? `&with_genres=${genre}`
          : ''}&region=US&with_release_type=3|2&primary_release_date.gte=${year}-${ ('0' + (
          index + 1)).slice(-2)}-01&primary_release_date.lte=${year}-${ ('0' + (
          index + 1)).slice(-2)}-31`;
        ...
        const monthDiv = movieDataDiv.append('div').attr('id', `${month}-${year}`).attr('class', 'month-container');

        const monthDivHeader = monthDiv.append('div').attr('class', 'month-header');
        ...
        table.append('thead').append('tr').selectAll('th').data($headers).enter().append('th').text(function(d) {
          return d === 'genre_ids'
            ? 'genres'
            : d.replace('_', ' ');
        ...
        // add table data
        table.append('tbody').selectAll('tr').data(filteredMovieData).enter().append('tr').selectAll('td').data(function(d) {
          return Object.keys(d).map((key) => d[key]);
        }).enter().append('td').attr('class', 'movie-table-body').text(function(d, index) {
        ...
        movieDataElement.selectAll('.lds-facebook').remove()
        movieDataDiv.append('h2').text(`${month}-${year} - Error Retrieving Data`);
      }
    });
  };
